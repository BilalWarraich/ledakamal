...
setAnchorPeer() {
  ORG=$1
  PEER=$2
  ${TEST_NETWORK_HOME}/scripts/setAnchorPeer.sh $ORG $CHANNEL_NAME $PEER
}

setGlobals bloodbank peer0
BLOCKFILE="${TEST_NETWORK_HOME}/channel-artifacts/${CHANNEL_NAME}.block"

echo "Fetching channel config block from orderer..."
set -x
peer channel fetch 0 $BLOCKFILE -o localhost:8050 --ordererTLSHostnameOverride orderer2.example.com -c $CHANNEL_NAME --tls --cafile "$ORDERER_CA" >&log.txt
res=$?
{ set +x; } 2>/dev/null
cat log.txt
...
joinChannel bloodbank peer0
joinChannel bloodbank peer1

infoln "Setting anchor peer for org3..."
setAnchorPeer bloodbank peer0
