...
joinChannel() {
  ORG=$1
  PEER=$2
  FABRIC_CFG_PATH=$PWD/../channel2-config/
  setGlobals $ORG $PEER
	local rc=1
	local COUNTER=1
...
setAnchorPeer() {
  ORG=$1
  PEER=$2
  . scripts/setAnchorPeer.sh $ORG $CHANNEL_NAME $PEER 
}


## User attempts to use BFT orderer in Fabric network with CA
if [ $BFT -eq 1 ] && [ -d "organizations/fabric-ca/orderer2Org/msp" ]; then
...
joinChannel hospital peer0
joinChannel hospital peer1

infoln "Joining org2 peer to the channel..."
joinChannel transportation peer0
joinChannel transportation peer1

## Set the anchor peers for each org in the channel
infoln "Setting anchor peer for org1..."
setAnchorPeer hospital peer0
infoln "Setting anchor peer for org2..."
setAnchorPeer transportation peer0
