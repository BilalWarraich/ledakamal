...
function generateOrg3Definition() {
  ...
  set -x
  configtxgen -printOrg bloodbankMSP > ../organizations/peerOrganizations/bloodbank.example.com/bloodbank.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function Org3Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/bloodbank.example.com/peers/peer0.bloodbank.example.com/
  fi
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/bloodbank.example.com" ]; then
    generateOrg3
