...
function approveForMyOrg() {
  ...
  setGlobals $ORG $PEER
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG}  --init-required --collections-config ${PWD}/../chaincode/blockchain/chaincode/collections_config.json --signature-policy "OR('bloodcollectionMSP.member','bloodprocessingMSP.member','bloodbankMSP.member')" >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
function checkCommitReadiness() {
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} --init-required --collections-config ${PWD}/../chaincode/blockchain/chaincode/collections_config.json --signature-policy "OR('bloodcollectionMSP.member','bloodprocessingMSP.member','bloodbankMSP.member')" --output json >&log.txt
    res=$?
    { set +x; } 2>/dev/null
    let rc=0
    for var in "$@"; do
...
function commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} "${PEER_CONN_PARMS[@]}" --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} --init-required --collections-config ${PWD}/../chaincode/blockchain/chaincode/collections_config.json --signature-policy "OR('bloodcollectionMSP.member','bloodprocessingMSP.member','bloodbankMSP.member')" >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
