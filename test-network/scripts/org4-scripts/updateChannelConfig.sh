...
set -x
jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"hospitalMSP":.[1]}}}}}' ${TEST_NETWORK_HOME}/channel-artifacts/config.json ${TEST_NETWORK_HOME}/organizations/peerOrganizations/hospital.example.com/connection-hospital.json > ${TEST_NETWORK_HOME}/channel-artifacts/modified_config.json
{ set +x; } 2>/dev/null
...
setGlobals bloodprocessing peer0 
set -x
